...
module.exports = function(RED) {
    ...
    function MQTTBrokerNode(n) {
        ...
        if (this.brokerurl === "") {
            ...
            if (this.broker !== "") {
                this.brokerurl = this.brokerurl+this.broker+"/mqtt";
            } else {
                this.brokerurl = this.brokerurl+"localhost/mqtt";
            }
        }
        ...
        if (proxy) {
            var parsed = url.parse(this.brokerurl)
...
// true for wss
            ...
            this.options.wsOptions =  wsOptions;
        }
        ...
        this.publish = function (msg) {
            if (node.connected) {
                ...
                node.client.publish(msg.topic, msg.payload, options, function (err){
                    node.log(RED._("mqtt.publishing"));
                    return});
            }
            else {
    ...
    RED.nodes.registerType("lo-mqtt-broker",MQTTBrokerNode,{
        credentials: {
            user: {type:"text"},
    ...
    RED.nodes.registerType("lo-mqtt in",MQTTInNode);
    ...
    RED.nodes.registerType("lo-mqtt out",MQTTOutNode);
};
